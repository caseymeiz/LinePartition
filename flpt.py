...
class FixedLength(object):
	...
	def segment(self, line, slice_size):
		...
		arcpy.AddMessage("="*80)
		while start < end:
			arcpy.AddMessage(str(start) +" "+ str(end))
			arcpy.AddMessage("   "+str(length))
			if end > length:
				end = length
			test = line.segmentAlongLine(start, end)
			if test.length == 0:
				break
			...
			if angle > 45:
				end = start + slice_size/np.cos(np.deg2rad(90-angle))
			elif angle < -45:
				end = start + slice_size/np.cos(np.deg2rad(90+angle))
			else:
				end = start + slice_size/np.cos(np.deg2rad(angle))
		...
		if len(segments) > 0:
			last_seg = segments[-1]
			line = arcpy.Polyline(arcpy.Array([last_seg.lastPoint, last_seg.firstPoint]))
